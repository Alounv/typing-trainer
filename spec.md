...
**Drill session graduation vs. classification change:** graduating a drill (per §4.1 session end trigger) means the bigram is done _for that session_. It does not change the bigram's classification — that only happens at the next diagnostic.
...
The user should never have to infer progress from raw numbers. The app's job is to interpret data and surface conclusions — _"you're getting better at X"_, _"Y is stuck"_, _"Z breakthrough happened this week"_ — backed by evidence. Numbers are evidence, not the message. Every metric exposed in the UI must answer "so what?" before it reaches the user.
...
- If session WPM drops more than 15% below the rolling average: explicitly attribute it — _"You're 18% below your average today. This often reflects fatigue or a harder-than-usual corpus selection."_ Never let a bad session feel like regression without context.
...
Not "days practiced" streaks (brittle, punishing). Instead: **improvement streaks** on specific bigrams — _"'th' has improved for 5 consecutive sessions"_. These are more meaningful and more robust.
...
- Surface explicitly: _"Your slowest bigrams haven't improved in 10 sessions. Here's what to try:"_
